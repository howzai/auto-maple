...
This tool does NOT press keys or control the game. It finds the visible MapleStory
window, captures only its client area, runs both trained models on the same frame,
and overlays detections so the two models can be validated together before they
are wired into automation decisions.
...
def _find_maple_window():
    ...
    def callback(hwnd, _extra):
        ...
        if not title:
            return
        if "楓之谷" in title or "maplestory" in folded or "新楓" in title:
    ...
    # Prefer the largest client area in case launchers or utility windows match.
    best = None
...
def _font(size: int = 18):
    # PIL default font is portable and sufficient for confidence text.
    try:
...
def _draw_box(draw, xyxy, text: str, outline, width: int = 3):
...
class CombinedVisionApp:
    def __init__(self, monster_conf: float, nav_conf: float, imgsz: int):
        try:
            import torch
    ...
    def tick(self):
        ...
        try:
            ...
            # MSS is BGRA. Convert to RGB for PIL and YOLO.
            frame = np.asarray(shot, dtype=np.uint8)[:, :, :3][:, :, ::-1].copy()
            ...
            # Fit preview to a practical desktop size without altering inference input.
        ...
        self.frame_index += 1
        # Yield to Tk. Inference itself limits the effective refresh rate.
...
def main() -> int:
    ...
    parser.add_argument("--monster-conf", type=float, default=0.50)
    parser.add_argument("--nav-conf", type=float, default=0.45)
